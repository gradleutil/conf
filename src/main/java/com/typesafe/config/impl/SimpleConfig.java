 ...
 * not "in" the map.
 */
public class SimpleConfig implements Config, MergeableValue, Serializable {
    ...
    public AbstractConfigValue find(Path pathExpression, ConfigValueType expected, Path originalPath) {
        return throwIfNull(findOrNull(object, pathExpression, expected, originalPath), expected, originalPath);
    }

    public AbstractConfigValue find(String pathExpression, ConfigValueType expected) {
        Path path = Path.newPath(pathExpression);
    ...
    private static void addMissing(List<ConfigException.ValidationProblem> accumulator,
        ...
        addProblem(accumulator, path, origin, "could not find set method for '" + path.render() + "', expecting: "
                   + refDesc);
    }
