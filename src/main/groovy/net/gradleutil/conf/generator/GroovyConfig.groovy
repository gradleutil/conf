...
class GroovyConfig {
    ...
    static String toGroovyDsl(Schema schema, String schemaName, String packageName) {
        ...
        def schemaRefName = schema.refValue.toString().replace('#/definitions/', '')
        ...
        schema.unprocessedProperties['definitions'].each { name, info ->
            ...
            info.properties.each { propName, propInfo ->
                ...
                def type = propInfoMap.type?.toString()?.capitalize() ?: propInfoMap['$ref'].toString().replace('#/definitions/', '')
                if (propInfoMap['$ref']) {
                    groovy.append("""
                        ...
                        ${type} set${type}( Config ${type.uncapitalize()}Config ) {
                            ${type.uncapitalize()} = new ${type}( config: ${type.uncapitalize()}Config )
                        }
                    ...
                    if (propInfoMap['patternProperties']) {
                        ...
                        groovy.append("""
                            ...
                            class ${propListType} extends Block {
                                void ${itemType.uncapitalize()}( @DelegatesTo( value = ${itemType}.class, strategy = Closure.DELEGATE_FIRST ) Closure<?> configurer ) {
                                    ${propName}.add( new ${itemType}( configurer: configurer ) )
                                }
                            }
                        """.stripIndent().replaceAll(/(?im)^/, spaces(2)))
                    } else {
